...
class Game {
  constructor() {
    ...
    this.disconnectTimeouts = new Map(); // customId → Timeout
    this.race = {
      isActive: false,
      startTime: null,
      finishTime: null,
  ...
  generateWorld() {
    this.trees = []; // Placeholder, tree images to be defined elsewhere
  ...
  startRace(byId) {
    ...
    setTimeout(() => {
      for (const pid in this.players) {
        if (!this.players[pid].disconnectedAt) {
          io.to(pid).emit('position-validated', { id: pid, x: 0, y: 0 });
          this.broadcastVisible(pid, 'player-moved', {
            id: pid,
            x: 0,
            y: 0,
...
io.on('connection', socket => {
  ...
  const safeEmit = (cb, ...args) => {
    try { cb(...args); }
      ...
      socket.emit('error-message', { message: 'Server error' });
      console.error(e);
    }
  };
  ...
  const sendInit = () => {
    const visible = {};
    for (const pid of game.visibility[socket.customId] || []) {
  ...
  socket.on('join-with-code', ({ code, name }, cb) => safeEmit(() => {
    ...
    if (!hostId || !game.players[hostId]) return cb({ success: false, message: 'Invalid code' });

    const display = (name?.trim()) || `Player-${socket.customId.slice(0, 5)}`;
  ...
  socket.on('restart-race', () => safeEmit(() => {
    ...
    if (gameHost !== socket.customId) return socket.emit('error-message', { message: 'Only host can restart' });
  ...
  socket.on('reconnect-to-race', cb => safeEmit(() => {
    ...
    if (!pl) return cb({ success: false, message: 'No active race' });

    const vp = Array.from(game.visibility[pid] || [])
  ...
  socket.on('disconnect', () => safeEmit(() => {
    ...
    if (player) {
      ...
      game.disconnectTimeouts.set(pid, timeout);
      for (const otherId of game.visibility[pid] || []) {
