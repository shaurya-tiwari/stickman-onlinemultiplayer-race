...
import socket, { visiblePlayers, safeEmit, updatePosition, lastKnownPosition } from "../socket"
...
const CanvasGame = ({ playerName, isHost, onError }) => {
  ...
  const autoRequestHostStatus = useCallback(
    (id) => {
      if (isHost && id) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (me && !me.isJumping) {
        ...
        const nearbyObstacles = obstacles.filter((ob) => Math.abs(ob.x - playerX) < proximityThreshold)
        ...
        <div className="relative" style={{ overflow: "hidden" }}>
          ...
          <div className="absolute -top-12 left-0 right-0 flex justify-center">
            <div className="bg-gradient-to-r from-purple-800 to-indigo-900 text-white px-6 py-2 rounded-t-xl shadow-lg border-t-2 border-l-2 border-r-2 border-indigo-500 transform -skew-x-6">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-orange-400 font-extrabold text-xl tracking-wider">
                STICK MAN RACING
              </span>
            </div>
          </div>
          ...
          <div className="absolute top-4 right-4 flex flex-col space-y-2">
            ...
            <div className="bg-black bg-opacity-70 backdrop-filter backdrop-blur-sm rounded-lg p-3 border border-indigo-500 flex flex-col space-y-2">
              ...
              {isRequestAccepter && (
                <div className="text-amber-300 text-xs mb-1 text-center">Controls disabled for request accepter</div>
              )}
